      ...
      <div className="container-shell flex h-14 items-center justify-between gap-4">
        <Link to="/" className="flex min-w-0 items-center gap-2.5">
          {siteSettings.logo_url ? (
            <img src={siteSettings.logo_url} alt={siteSettings.business_name} className="h-9 w-9 rounded-xl object-cover" />
          ) : (
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-700 text-xs font-extrabold text-white">{logoLabel}</div>
          )}
          <div className="min-w-0">
            <p className="truncate text-[13px] font-extrabold tracking-tight text-ink-900 sm:text-sm">{siteSettings.business_name}</p>
            <p className="truncate text-xs text-ink-500">{siteSettings.tagline}</p>
          </div>
        </Link>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <Link to="/parts" className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-stone-100 text-ink-700 transition hover:bg-stone-200">
